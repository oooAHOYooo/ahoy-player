...
pub fn run() -> Result<()> {
    ...
    window.on_import_files(move || { if let Some(window) = weak.upgrade() { let mut state = state_import.borrow_mut(); let mut imported = 0; let mut duplicates = 0; let mut failures = 0; for path in pick_mp3s() { match state.library.import(path) { Ok(ImportResult::Imported) => imported += 1, Ok(ImportResult::Duplicate) => duplicates += 1, Ok(ImportResult::Rejected) | Err(_) => failures += 1 } } if imported > 0 { state.queue = QueueState::new(state.library.tracks.iter().map(|track| track.id.clone()).collect()); if let Err(error) = state.library.save(&state.data_dir.join("library.json")) { window.set_status(format!("Could not save library: {error}").into()); } else { refresh(&window, &state); } } let message = if imported > 0 { format!("Imported {imported}; {duplicates} duplicate(s); {failures} failed") } else if duplicates > 0 { format!("No new tracks; {duplicates} duplicate(s)") } else if failures > 0 { format!("Import failed for {failures} file(s)") } else { "Import cancelled".to_string() }; window.set_status(message.into()); } });
