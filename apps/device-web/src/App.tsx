...
export function App() {
  ...
  const [workspaceView, setWorkspaceView] = useState<"library" | "customize" | "moods">("library");
  ...
  return <main className={`app-shell theme-${theme} ${ambientGlow ? "has-ambient-glow" : ""} ${sidebarOpen ? "" : "sidebar-collapsed"} ${inspectorOpen ? "inspector-open" : ""}`} style={shellStyle}>
    <aside className="sidebar">
      ...
      <div className="playlist-links"><button type="button" onClick={() => setWorkspaceView("library")}>◈  All music</button><button type="button" className={workspaceView === "moods" ? "is-customize-active" : ""} onClick={() => setWorkspaceView("moods")}>◌  Open archive moods</button><button type="button">＋  New playlist</button><button type="button" className={workspaceView === "customize" ? "is-customize-active" : ""} onClick={() => setWorkspaceView("customize")}>⌘  Customize</button></div>
...
function MoodsView({ packs, onDownload, onBack }: { packs: FreeMusicPack[]; onDownload: (pack: FreeMusicPack) => void; onBack: () => void }) {
  return <div className="moods-screen"><div className="moods-heading"><div><p className="eyebrow">OPEN ARCHIVE MOODS</p><h1>Find a<br /><em>starting point.</em></h1><p className="hero-copy">Curated public-domain and open-archive listening lists, kept separate from your local music library.</p></div><button className="outline-button" type="button" onClick={onBack}>← Back to library</button></div><FreePacks packs={packs} onDownload={onDownload} /></div>;
}
